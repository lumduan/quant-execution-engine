...
A ~30 s low-impact probe: success resets the breaker, consecutive failures at
the threshold trip it OPEN, and the CLOSED→OPEN *transition* fires ``on_trip``
exactly once (the runtime wires it to the router's mass-cancel sweep). The
loop shell is deliberately thin — ``heartbeat_pass`` is the tested unit.
"""
...
async def heartbeat_pass(adapter: LiberatorAdapter, *, on_trip: TripHook) -> bool:
    ...
    if ok:
        if was_open:
            logger.warning("liberator session recovered; circuit breaker resetting")
    ...
    if adapter.breaker.state is BreakerState.OPEN and not was_open:
        logger.error("liberator circuit breaker TRIPPED — halting routing, mass-cancel attempted")
        try:
            await on_trip()
        except Exception:  # noqa: BLE001 - the heartbeat loop must survive the hook
            logger.exception("liberator breaker on_trip hook failed")
    return False
    ...
    *,
    ...
    while True:  # pragma: no branch - cancelled via task.cancel()
        try:
            ...
            logger.exception("liberator heartbeat pass failed unexpectedly")
