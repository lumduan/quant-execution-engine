...
```json
{
  ...
  "brokers": {
    ...
    "settrade": {
      ...
      "sessions": { "SET": true, "TFEX": true }
    }
  },
  "order_book": {
    "active_provider": "liberator",
    "providers": ["liberator", "settrade"],
    "cached_symbols": 12,
    "subscribers": 3
  }
}
...
| `brokers` | object \| null | `null` when broker-free; otherwise one entry per configured broker |
...
| `brokers.settrade.sessions` | object \| null | per-market last heartbeat (Phase 4.1), e.g. `{"SET": true, "TFEX": null}` |
